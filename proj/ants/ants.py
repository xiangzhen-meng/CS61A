...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8b
            origin = place.ant
            if origin.can_contain(self):
                assert origin.is_container == True
                origin.store_ant(self)
            elif self.can_contain(origin):
                assert self.is_container == True
                self.store_ant(origin)
                place.ant = self
            else:
...
class ThrowerAnt(Ant):
    ...
    # upper_bound means not bigger than upper_bound, specially for ShortThrower
    lower_bound = 0
    # lower_bound means not smaller than lower_bound, specially for LongThrower
    def __init__(self, health=1):
        super().__init__(1)
    ...
    def nearest_bee(self):
        ...
        def find(cur_place: Place, dist_cnt):
            if cur_place.is_hive or dist_cnt > self.upper_bound:
                return None
            if (not cur_place.bees) or (dist_cnt < self.lower_bound):
                return find(cur_place.entrance, dist_cnt + 1)
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        assert self.place.bees != None
        cur_bees = list(self.place.bees)
        super().reduce_health(amount)
        cur_damage = amount
        if self.place == None:
            cur_damage += self.damage
        for i in range(len(cur_bees)):
            cur_bees[i].reduce_health(cur_damage)
        # END Problem 5
...
class WallAnt(Ant):
    """
    WallAnt does nothing but has a large amount of initial health
    """
    name = 'Wall'
    implemented = True
    food_cost = 4
...
class HungryAnt(Ant):
    """
    HungryAnt will select a random Bee from its place and deal damage to the Bee 
    equal to the Bee's health, eating it whole.
    """
    name = 'Hungry'
    food_cost = 4
    chew_cooldown = 3
    ...
    def action(self, gamestate):
        if self.cooldown > 0:
            ...
            tar_health = target.health
            target.reduce_health(tar_health)
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        ...
        "*** YOUR CODE HERE ***"
        # init slow_times
        target.slow_times = self.slow_turn
        
        # for the syruped bee to take action
        def jump_action(gamestate):
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        # BEGIN Problem EC 4
        front = {}
        def find_insect(cur_place: Place, dist: int):
            if cur_place.is_hive:
                return
            # find ants
            if cur_place == self.place:
                if cur_place.ant.is_container:
                    front[cur_place.ant] = 0   
            elif cur_place.ant != None:
                front[cur_place.ant] = dist
                if cur_place.ant.is_container and cur_place.ant.ant_contained:
                    front[cur_place.ant.ant_contained] = dist
            # find bees
            cur_bees = list(cur_place.bees)
            for b in cur_bees:
                front[b] = dist
            # find nxt place
            find_insect(cur_place.entrance, dist + 1)
        find_insect(self.place, 0)
        return front
        # END Problem EC 4

    def calculate_damage(self, distance):
        # BEGIN Problem EC 4
        tot_weaken = self.dist_weaken * distance + self.battery_weaken * self.insects_shot
        if tot_weaken >= self.damage:
            return 0
        else:
            return self.damage - tot_weaken
        # END Problem EC 4
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        start = self.place.entrance

        # if scared times left:
        if hasattr(self, "scared_left") and self.scared_left > 0:
            self.scared_left -= 1
            if self.blocked():
    ...
    def scare(self, length):
        ...
        # BEGIN Problem EC 2
        "*** YOUR CODE HERE ***"
        if not hasattr(self, "scared_left"):
            self.scared_left = length
        # END Problem EC 2
