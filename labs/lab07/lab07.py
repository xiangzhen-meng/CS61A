...
def without(s, i):
    ...
    "*** YOUR CODE HERE ***"
    head = Link(0)
    def new_without(pre_h, cur_s, dep):
        if cur_s == Link.empty:
            return
        if dep == i:
            new_without(pre_h, cur_s.rest, dep + 1)
        else:
            tmp = Link(cur_s.first)
            pre_h.rest = tmp
            new_without(tmp, cur_s.rest, dep + 1)
    new_without(head, s, 0)
    return head.rest
