...
def calc_eval(exp):
    ...
    if isinstance(exp, Pair):
        ...
        if operator == 'and': # and expressions
            ...
            return calc_eval(Pair(operator, operands))
...
def floor_div(args):
    ...
    if isinstance(args.first, Pair):
        init_dividend = calc_eval(args.first)
    else:
        init_dividend = args.first
    def cal(dividend: int, cur):
        if cur is nil:
            return dividend
        if isinstance(cur.first, Pair):
            divisor = calc_eval(cur.first)
        else:
            divisor = cur.first
        dividend = dividend // divisor
        return cal(dividend, cur.rest)
    return cal(init_dividend, args.rest)
