...
def balanced(m):
    ...
    if is_planet(m):
        return True
    # recursive cases
    lft_arm, rgt_arm = left(m), right(m) 
    lft_mob, rgt_mob = end(lft_arm), end(rgt_arm)
    lft_len, rgt_len = length(lft_arm), length(rgt_arm)
    lft_mass, rgt_mass = total_mass(lft_mob), total_mass(rgt_mob)
    if not (balanced(lft_mob) and balanced(rgt_mob)):
        return False
    if lft_mass * lft_len != rgt_mass * rgt_len:
        return False
    return True
...
def max_path_sum(t):
    ...
    if is_leaf(t):
        return label(t)
    branch = branches(t)
    branch_max = [max_path_sum(b) for b in branch]
    return label(t) + max(branch_max)
