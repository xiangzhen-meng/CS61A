...
def count_dollars(total):
    ...
    "*** YOUR CODE HERE ***"
    def recur(tot, maxim):
        nxt = next_smaller_dollar(maxim)
        if tot == 0:
            return 1
        elif tot < 0:
            return 0
        elif not nxt:
            with_max = recur(tot - maxim, maxim)
            return with_max
        else:
            with_max = recur(tot - maxim, maxim)
            without_max = recur(tot, nxt)
...
def count_dollars_upward(total):
    ...
    "*** YOUR CODE HERE ***"
    def recur(tot, minim):
        nxt = next_larger_dollar(minim)
        if tot == 0:
            return 1
        elif tot < 0:
            return 0
        elif not nxt:
            with_min = recur(tot - minim, minim)
            return with_min
        else:
            with_min = recur(tot - minim, minim)
            without_min = recur(tot, nxt)
